...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $updateQuery = "UPDATE dinning SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
...
// Fetch existing data
$dataFetchQuery = "SELECT * FROM dinning WHERE id=?";
...
<body>
    <div class="container mt-4">
        <div class="upload-section">
            ...
            <?php endif; ?>
            <form action="dinning_up_action.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="furnitureType" name="furnitureType" required>
                        ...
                        <option value="Dining Table" <?php if ($data['type'] === 'Dining Table')
                            echo 'selected'; ?>>
                            Dining Table</option>
                        <option value="Dining Chair" <?php if ($data['type'] === 'Dining Chair')
                            echo 'selected'; ?>>
                            Dining Chair</option>
                        <option value="Dining Set" <?php if ($data['type'] === 'Dining Set')
                            echo 'selected'; ?>>
                            Dining Set</option>
                        <option value="Dinner Wagon" <?php if ($data['type'] === 'Dinner Wagon')
                            echo 'selected'; ?>>
                           Dinner Wagon</option>
                ...
                <div class="form-group">
                    ...
                    <div id="existingImages">
                        <?php
                        // Fetch existing images associated with the dinning furniture ID
                        $fetchImagesQuery = "SELECT image_path FROM dinning_images WHERE furniture_id = ?";
