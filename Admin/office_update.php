...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $updateQuery = "UPDATE office SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
...
// Fetch existing data
$dataFetchQuery = "SELECT * FROM office WHERE id=?";
...
<body>
    <div class="container mt-4">
        <div class="upload-section">
            ...
            <?php endif; ?>
            <form action="office_up_action.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="furnitureType" name="furnitureType" required>
                        <option value="Executive Table" <?php if ($data['type'] === 'Executive Table')
                            echo 'selected'; ?>>Executive Table</option>
                        <option value="Senior Ex Table" <?php if ($data['type'] === 'Senior Ex Table')
                            echo 'selected'; ?>>
                            Senior Ex Table</option>
                        <option value="Swivel Chair" <?php if ($data['type'] === 'Swivel Chair')
                            echo 'selected'; ?>>
                            Swivel Chair</option>
                        <option value="Visitor Chair" <?php if ($data['type'] === 'Visitor Chair')
                            echo 'selected'; ?>>
                            Visitor Chair</option>
                        <option value="Waiting Chair" <?php if ($data['type'] === 'Waiting Chair')
                            echo 'selected'; ?>>
                           Waiting Chair</option>
                        <option value="Waiting Sofa" <?php if ($data['type'] === 'Waiting Sofa')
                            echo 'selected'; ?>>
                            Waiting Sofa</option>
                        <option value="Side Table" <?php if ($data['type'] === 'Side Table')
                            echo 'selected'; ?>>
                            Side Table</option>
                ...
                <div class="form-group">
                    ...
                    <div id="existingImages">
                        <?php
                        // Fetch existing images associated with the office furniture ID
                        $fetchImagesQuery = "SELECT image_path FROM office_images WHERE furniture_id = ?";
