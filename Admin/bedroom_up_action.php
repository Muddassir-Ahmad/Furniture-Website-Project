...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updateBedroomQuery = "UPDATE bedroom SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
            $stmtBedroom = mysqli_prepare($conn, $updateBedroomQuery);
            mysqli_stmt_bind_param($stmtBedroom, "sssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $materials, $size, $information, $id);
            mysqli_stmt_execute($stmtBedroom);

            if ($updateImages) {
                // If images are updated, handle insertion and deletion logic as before
                $deleteImagesQuery = "DELETE FROM bedroom_images WHERE furniture_id = ?";
                ...
                $insertImageQuery = "INSERT INTO bedroom_images (furniture_id, image_path) VALUES (?, ?)";
    ...
    header("Location: bedroom_update.php?id=$id");
    exit();
}
?>