...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updatelivingQuery = "UPDATE living SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
            $stmtliving = mysqli_prepare($conn, $updatelivingQuery);
            mysqli_stmt_bind_param($stmtliving, "sssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $materials, $size, $information, $id);
