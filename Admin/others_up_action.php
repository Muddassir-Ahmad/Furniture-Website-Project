...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updateothersQuery = "UPDATE others SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
            $stmtothers = mysqli_prepare($conn, $updateothersQuery);
            mysqli_stmt_bind_param($stmtothers, "sssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $materials, $size, $information, $id);
