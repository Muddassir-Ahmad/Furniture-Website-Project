...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $updateQuery = "UPDATE reading SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
...
// Fetch existing data
$dataFetchQuery = "SELECT * FROM reading WHERE id=?";
...
<body>
    <div class="container mt-4">
        <div class="upload-section">
            <h2>Update Reading Furniture</h2>
            <?php if (!empty($errors)): ?>
            ...
            <?php endif; ?>
            <form action="reading_up_action.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="furnitureType" name="furnitureType" required>
                        <option value="Bookshelf" <?php if ($data['type'] === 'Bookshelf')
                            echo 'selected'; ?>>Bookshelf</option>
                        <option value="Reading Table" <?php if ($data['type'] === 'Reading Table')
                            echo 'selected'; ?>>
                            Reading Table</option>
                        <option value="Computer Table" <?php if ($data['type'] === 'Computer Table')
                            echo 'selected'; ?>>
                            Computer Table</option>
                        <option value="Computer Chair" <?php if ($data['type'] === 'Computer Chair')
                            echo 'selected'; ?>>
                            Computer Chair</option>
                        <option value="Reading Chair" <?php if ($data['type'] === 'Reading Chair')
                            echo 'selected'; ?>>
                           Reading Chair</option>
                ...
                <div class="form-group">
                    ...
                    <div id="existingImages">
                        <?php
                        // Fetch existing images associated with the reading furniture ID
                        $fetchImagesQuery = "SELECT image_path FROM reading_images WHERE furniture_id = ?";
