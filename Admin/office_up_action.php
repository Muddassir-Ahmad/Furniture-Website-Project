...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updateofficeQuery = "UPDATE office SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
            $stmtoffice = mysqli_prepare($conn, $updateofficeQuery);
            mysqli_stmt_bind_param($stmtoffice, "sssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $materials, $size, $information, $id);
            mysqli_stmt_execute($stmtoffice);

            if ($updateImages) {
                // If images are updated, handle insertion and deletion logic as before
                $deleteImagesQuery = "DELETE FROM office_images WHERE furniture_id = ?";
                ...
                $insertImageQuery = "INSERT INTO office_images (furniture_id, image_path) VALUES (?, ?)";
    ...
    header("Location: office_update.php?id=$id");
    exit();
}
?>