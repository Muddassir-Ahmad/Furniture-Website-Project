...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $updateQuery = "UPDATE others SET name=?, code=?, type=?, price=?, material=?, size=?, information=? WHERE id=?";
...
// Fetch existing data
$dataFetchQuery = "SELECT * FROM others WHERE id=?";
...
<body>
    <div class="container mt-4">
        <div class="upload-section">
            ...
            <?php endif; ?>
            <form action="others_up_action.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="furnitureType" name="furnitureType" required>
                        <option value="Rack" <?php if ($data['type'] === 'Rack')
                            echo 'selected'; ?>>Rack</option>
                        <option value="Side Box" <?php if ($data['type'] === 'Side Box')
                            echo 'selected'; ?>>
                            Side Box</option>
                        <option value="Mattress" <?php if ($data['type'] === 'Mattress')
                            echo 'selected'; ?>>
                            Mattress</option>
                        <option value="Office Set" <?php if ($data['type'] === 'Office Set')
                            echo 'selected'; ?>>
                            Office Set</option>
                        <option value="Home Decor" <?php if ($data['type'] === 'Home Decor')
                            echo 'selected'; ?>>
                           Home Decor</option>
                        <option value="Wedding Set" <?php if ($data['type'] === 'Wedding Set')
                            echo 'selected'; ?>>
                            Wedding Set</option>
                        <option value="Tea Table" <?php if ($data['type'] === 'Tea Table')
                            echo 'selected'; ?>>
                            Tea Table</option>
                ...
                <div class="form-group">
                    ...
                    <div id="existingImages">
                        <?php
                        // Fetch existing images associated with the others furniture ID
                        $fetchImagesQuery = "SELECT image_path FROM others_images WHERE furniture_id = ?";
